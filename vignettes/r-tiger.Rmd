...
# Genotyping seven sequencing samples from Arabidopsis Thaliana
...
file.example = read.delim(files[1], header = F)
head(file.example)
```
...
```{r initializing mydat, include =TRUE, cache= TRUE}
...
```{r fitting model, cache = TRUE, warning=FALSE}
...
```{r rviterbi, cache = T, warning=FALSE}
